...
- `§0-7-d-v` Large implementation files still exceed maintainable review size and concentrate unrelated ownership. **[high]** `[~]`
  - *In progress*: `scripting/src/deferred_mutations.cpp` now owns the deferred world-mutation queue, phase gate, apply-or-queue helpers, flush path, and shutdown clear; `scripting/src/timer_bindings.cpp` owns Lua timer refs and callback rewiring; `scripting/src/collision_bindings.cpp` owns Lua collision callback refs and dispatch; `scripting/src/persist_bindings.cpp` owns the Lua hot-reload persist table; `scripting/src/touch_bindings.cpp` owns Lua touch/gesture callbacks; `scripting/src/coroutine_bindings.cpp` owns Lua coroutine scheduler refs, wait tags, and tick/clear behavior; `scripting/src/game_bindings.cpp` owns game mode/state strings, persistent game state, and player-controller slots; `scripting/src/debug_bindings.cpp` owns debugger/profiler hook state; `scripting/src/entity_script_bindings.cpp` owns entity script module cache and lifecycle dispatch; `scripting/src/lua_state.cpp` owns Lua state lifetime; `scripting/src/input_bindings.cpp` owns keyboard, action, gamepad, input-map, and key-constant Lua bindings; `scripting/src/scene_bindings.cpp` owns pending scene operations and Lua scene save/load/new bindings; `scripting/src/entity_handle.cpp` owns Lua entity-handle encoding, decoding, liveness reads, and push helpers; `scripting/src/entity_pool_bindings.cpp` owns Lua-created entity pools and their reset path; `renderer/src/render_settings.cpp` owns renderer fog/reflection-probe setting parsing and normalization; `renderer/src/command_buffer_builder.cpp` owns command-buffer sorting and static mesh batch construction; `renderer/src/command_buffer_math.cpp` owns draw matrix helpers and directional shadow cache-key hashing; `renderer/src/command_buffer_context.cpp/.h` own renderer context and backend state storage; `engine_unit_scripting` verifies queued mutations/debugger/profiler/module behavior, `engine_unit_input_map` verifies input-map behavior, `engine_integration_coroutine` covers the coroutine scheduler behavior, `engine_integration_game_mode` covers the game/player state behavior, `engine_integration_lua_lifecycle` covers entity lifecycle dispatch, `engine_integration_dap` covers debugger DAP behavior, and `engine_unit_command_buffer` covers renderer setting normalization and command sorting/batching.
  - *Audit*: Current largest files still include `command_buffer.cpp` (~3772 after the render settings, builder, math, and context splits), `scripting.cpp` (~3164 after the timer/collision/persist/touch/coroutine/game/debug/entity-script/state/input/scene/entity-handle/entity-pool split), `editor.cpp` (~2348), `physics.cpp` (~2278), `tests/unit/physics_test.cpp` (~2129), `engine_pipeline.cpp` (~2007), `world.cpp` (~1703), and `scene_serializer.cpp` (~1342). The extracted `command_buffer_context.h` is ~352 lines, `command_buffer_builder.cpp` is ~178 lines, `command_buffer_math.cpp` is ~91 lines, `input_bindings.cpp` is ~384 lines, `scene_bindings.cpp` is ~76 lines, `entity_handle.cpp` is ~95 lines, and `entity_pool_bindings.cpp` is ~94 lines.
