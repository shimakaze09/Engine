...
- `§0-7-d-i` `scripting/src/scripting.cpp` remains a 5K+ line global-state owner for Lua state, debugger/profiler state, and entity script modules. **[critical]** `[~]`
...
- `§0-7-d-v` Large implementation files still exceed maintainable review size and concentrate unrelated ownership. **[high]** `[~]`
  - *In progress*: `scripting/src/deferred_mutations.cpp` now owns the deferred world-mutation queue, phase gate, apply-or-queue helpers, flush path, and shutdown clear; `scripting/src/timer_bindings.cpp` owns Lua timer refs and callback rewiring; `scripting/src/collision_bindings.cpp` owns Lua collision callback refs and dispatch; `scripting/src/persist_bindings.cpp` owns the Lua hot-reload persist table; `scripting/src/touch_bindings.cpp` owns Lua touch/gesture callbacks; `scripting/src/coroutine_bindings.cpp` owns Lua coroutine scheduler refs, wait tags, and tick/clear behavior; `scripting/src/game_bindings.cpp` owns game mode/state strings, persistent game state, and player-controller slots; `engine_unit_scripting` verifies queued mutations are discarded across scripting shutdown/reinitialization, `engine_integration_coroutine` covers the coroutine scheduler behavior, and `engine_integration_game_mode` covers the game/player state behavior.
  - *Audit*: Current largest files still include `scripting.cpp` (~5156 lines after the timer/collision/persist/touch/coroutine/game binding split), `command_buffer.cpp` (~4509), `editor.cpp` (~2341), `physics.cpp` (~2147), `engine_pipeline.cpp` (~1790), `world.cpp` (~1702), and `scene_serializer.cpp` (~1320).
