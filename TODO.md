...
- `§0-7-d-v` ~~Large implementation files still exceed maintainable review size and concentrate unrelated ownership.~~ **[high]** `[x]`
  ...
  - *Audit (refreshed 2026-07-17, campaign complete)*: Current largest files are `tests/unit/physics_test.cpp` (2590), `physics.cpp` (2479, after the A4 narrow-phase extraction), `command_buffer_flush.cpp` (2050, the frame-flush TU from the A1 split), `engine_pipeline.cpp` (1928), and `world.cpp` (1650). The former monoliths are gone: `scripting.cpp` is 676 lines and `editor.cpp` 329 after the A3 domain/panel splits; `command_buffer.cpp` is 1297 after A1. All remaining large files are single-domain implementation surfaces; no size-only decomposition is tracked.
