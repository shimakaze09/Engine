...
bool can_apply_mutations_now() noexcept {
  return (runtime_binding().world != nullptr) && (runtime_binding().services != nullptr) &&
         (runtime_binding().services->get_current_phase(runtime_binding().world) == runtime::WorldPhase::Input);
}
...
bool apply_or_queue_destroy_entity(runtime::Entity entity) noexcept {
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().services->destroy_entity_op(runtime_binding().world, entity.index);
  }
...
bool apply_or_queue_transform(runtime::Entity entity,
  ...
  if (can_apply_mutations_now()) {
    const bool transformUpdated =
        runtime_binding().services->add_transform_op(runtime_binding().world, entity.index, transform);
    if (!transformUpdated) {
      return false;
    }
    return !setAuthority || runtime_binding().services->set_movement_authority_op(
                                runtime_binding().world, entity.index, authority);
  }
...
bool apply_or_queue_rigid_body(runtime::Entity entity,
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().services->add_rigid_body_op(runtime_binding().world, entity.index, rigidBody);
  }
...
bool apply_or_queue_collider(runtime::Entity entity,
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().services->add_collider_op(runtime_binding().world, entity.index, collider);
  }
...
bool apply_or_queue_mesh_component(
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().services->add_mesh_component_op(runtime_binding().world, entity.index, component);
  }
...
bool apply_or_queue_name_component(
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().services->add_name_component_op(runtime_binding().world, entity.index, component);
  }
...
bool apply_or_queue_light_component(
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().services->add_light_component_op(runtime_binding().world, entity.index, component);
  }
...
bool apply_or_queue_remove_light_component(runtime::Entity entity) noexcept {
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().services->remove_light_component_op(runtime_binding().world, entity.index);
  }
...
bool apply_or_queue_script_component(
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().services->add_script_component_op(runtime_binding().world, entity.index,
                                               component);
  }
...
bool apply_or_queue_remove_script_component(runtime::Entity entity) noexcept {
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().services->remove_script_component_op(runtime_binding().world, entity.index);
  }
...
bool apply_or_queue_point_light_component(
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().world->add_point_light_component(entity, component);
  }
...
bool apply_or_queue_remove_point_light_component(
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().world->remove_point_light_component(entity);
  }
...
bool apply_or_queue_spot_light_component(
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().world->add_spot_light_component(entity, component);
  }
...
bool apply_or_queue_remove_spot_light_component(
  ...
  if (can_apply_mutations_now()) {
    return runtime_binding().world->remove_spot_light_component(entity);
  }
...
bool read_entity(lua_State *state, int index,
  ...
  const std::uint32_t validIndex =
      runtime_binding().services->get_entity_index(runtime_binding().world, entityIndex);
  if (validIndex == 0U) {
    return false;
  }

  const runtime::Entity resolved = runtime_binding().world->find_entity_by_index(validIndex);
...
int lua_engine_is_alive(lua_State *state) noexcept {
  ...
  const runtime::Entity entity = runtime_binding().world->find_entity_by_index(entityIndex);
...
int lua_engine_push_camera(lua_State *state) noexcept {
  ...
  const bool ok =
      runtime_binding().services->push_camera_op(runtime_binding().world, entityIdx, posX, posY, posZ, tgtX,
...
int lua_engine_pop_camera(lua_State *state) noexcept {
  ...
  const auto entityIdx =
      static_cast<std::uint32_t>(luaL_checkinteger(state, 1));
  const bool ok = runtime_binding().services->pop_camera_op(runtime_binding().world, entityIdx);
  lua_pushboolean(state, ok ? 1 : 0);
  return 1;
}
...
int lua_engine_add_spring_arm(lua_State *state) noexcept {
  ...
  const runtime::Entity entity = runtime_binding().world->find_entity_by_index(entityIdx);
...
void lua_timer_callback(runtime::TimerId id, void *userData) noexcept {
  ...
  if (runtime_binding().world != nullptr) {
    const auto &entry = runtime_binding().world->timer_manager().entry_at(slot);
    if (!entry.active) {
...
bool bindable_is_alive(std::uint32_t entity) noexcept {
  ...
  const runtime::Entity resolved = runtime_binding().world->find_entity_by_index(entity);
...
bool bindable_has_light(std::uint32_t entity) noexcept {
  ...
  return runtime_binding().world->has_light_component(resolved);
}
...
void flush_deferred_mutations() noexcept {
  ...
  for (std::size_t i = 0U; i < count; ++i) {
    ...
    case DeferredMutationType::DestroyEntity:
      static_cast<void>(
          runtime_binding().services->destroy_entity_op(runtime_binding().world, mutation.entity.index));
      break;
    case DeferredMutationType::SetTransform: {
      const bool transformUpdated = runtime_binding().services->add_transform_op(
          runtime_binding().world, mutation.entity.index, mutation.transform);
      if (transformUpdated && mutation.setMovementAuthority) {
        static_cast<void>(runtime_binding().services->set_movement_authority_op(
            runtime_binding().world, mutation.entity.index, mutation.movementAuthority));
      }
      break;
    }
    case DeferredMutationType::AddRigidBody:
      static_cast<void>(runtime_binding().services->add_rigid_body_op(
          runtime_binding().world, mutation.entity.index, mutation.rigidBody));
      break;
    case DeferredMutationType::AddCollider:
      static_cast<void>(runtime_binding().services->add_collider_op(
          runtime_binding().world, mutation.entity.index, mutation.collider));
      break;
    case DeferredMutationType::AddMeshComponent:
      static_cast<void>(runtime_binding().services->add_mesh_component_op(
          runtime_binding().world, mutation.entity.index, mutation.meshComponent));
      break;
    case DeferredMutationType::AddNameComponent:
      static_cast<void>(runtime_binding().services->add_name_component_op(
          runtime_binding().world, mutation.entity.index, mutation.nameComponent));
      break;
    case DeferredMutationType::AddLightComponent:
      static_cast<void>(runtime_binding().services->add_light_component_op(
          runtime_binding().world, mutation.entity.index, mutation.lightComponent));
      break;
    case DeferredMutationType::RemoveLightComponent:
      static_cast<void>(runtime_binding().services->remove_light_component_op(
          runtime_binding().world, mutation.entity.index));
      break;
    case DeferredMutationType::AddScriptComponent:
      static_cast<void>(runtime_binding().services->add_script_component_op(
          runtime_binding().world, mutation.entity.index, mutation.scriptComponent));
      break;
    case DeferredMutationType::RemoveScriptComponent:
      static_cast<void>(runtime_binding().services->remove_script_component_op(
          runtime_binding().world, mutation.entity.index));
      break;
    case DeferredMutationType::AddPointLightComponent: {
      const runtime::Entity resolved =
          runtime_binding().world->find_entity_by_index(mutation.entity.index);
      if (resolved != runtime::kInvalidEntity) {
        static_cast<void>(runtime_binding().world->add_point_light_component(
    ...
    case DeferredMutationType::RemovePointLightComponent: {
      const runtime::Entity resolved =
          runtime_binding().world->find_entity_by_index(mutation.entity.index);
      if (resolved != runtime::kInvalidEntity) {
        static_cast<void>(runtime_binding().world->remove_point_light_component(resolved));
      }
      break;
    }
    case DeferredMutationType::AddSpotLightComponent: {
      const runtime::Entity resolved =
          runtime_binding().world->find_entity_by_index(mutation.entity.index);
      if (resolved != runtime::kInvalidEntity) {
        static_cast<void>(runtime_binding().world->add_spot_light_component(
    ...
    case DeferredMutationType::RemoveSpotLightComponent: {
      const runtime::Entity resolved =
          runtime_binding().world->find_entity_by_index(mutation.entity.index);
      if (resolved != runtime::kInvalidEntity) {
        static_cast<void>(runtime_binding().world->remove_spot_light_component(resolved));
      }
      break;
    }
    }
