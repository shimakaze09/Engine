...
bool scripting_raycast(runtime::World *world, float ox, float oy, float oz,
                       ...
                       scripting::RaycastHit *outHit) noexcept {
  if ((world == nullptr) || (outHit == nullptr)) {
    return false;
  }

  physics::RayHit hit{};
  if (!physics::raycast(*world, math::Vec3(ox, oy, oz), math::Vec3(dx, dy, dz),
                        maxDistance, &hit)) {
    return false;
  }
...
std::uint32_t scripting_add_distance_joint(runtime::World *world,
  ...
  if ((entityA == runtime::kInvalidEntity) ||
      (entityB == runtime::kInvalidEntity)) {
    return 0U;
  }
  ...
  return static_cast<std::uint32_t>(physics::add_joint(desc));
}

void scripting_remove_joint(std::uint32_t jointId) noexcept {
  physics::remove_joint(static_cast<physics::JointId>(jointId));
}
...
void scripting_wake_body(runtime::World *world,
  ...
  physics::wake_body(*world, world->find_entity_by_index(entityIndex));
}
...
bool scripting_is_sleeping(runtime::World *world,
  ...
  return physics::is_sleeping(*world, world->find_entity_by_index(entityIndex));
}
...
const scripting::Services kScriptingServices = {
...
void run(std::uint32_t maxFrames) noexcept {
  ...
  scripting::set_services(&kScriptingServices);
