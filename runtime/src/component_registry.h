...
// name, and the World get/add accessor pair. world_component_counts_match and
// copy_world_contents in scene_serializer.cpp and the codec cross-validation
// suite (tests/unit/component_registry_test.cpp) expand this table, and the
// static_asserts below fail the build whenever the table and
...
// row cannot outlive its World type.
...
// Row order matches World::PersistentComponentTypes exactly (asserted below).
#define ENGINE_PERSISTENT_COMPONENT_TABLE(X)                                   \
  ...
  X(Collider, kJsonKeyCollider, get_collider, add_collider)                    \
...
#define ENGINE_PCR_TUPLE_ELEM(Type, Key, GetFn, AddFn)                         \
...
// Every row's accessor pair must keep the canonical get/add signatures the
// generated copy, count-check, and round-trip code depends on.
#define ENGINE_PCR_CHECK_ROW(Type, Key, GetFn, AddFn)                          \
