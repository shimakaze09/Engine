...
int main() {
  ...
  const char *scriptContents =
      ...
      "    end\n"
      "    -- The child's own descendant cannot become its parent (#531).\n"
      "    if engine.set_parent(parent, kid) then\n"
      "        error('cycle-closing set_parent must fail')\n"
      "    end\n"
      "    if engine.get_parent(parent) ~= nil then\n"
      "        error('refused reparent must leave the parent a root')\n"
      "    end\n"
