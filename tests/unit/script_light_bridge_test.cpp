...
constexpr const char *kScript =
    ...
    "    end\n"
    "    local r, g, b, i, rad, cast = engine.get_point_light(g_lit)\n"
    "    if r ~= 0.25 or g ~= 0.5 or b ~= 0.75 or i ~= 2.0 or rad ~= 7.0 then\n"
    "        error('point light did not read back')\n"
    "    end\n"
    "    if cast ~= false then error('point light cast_shadow default') end\n"
    "    if not engine.set_point_light(g_lit, 0.25, 0.5, 0.75, 2.0, 7.0,\n"
    "                                  true) then\n"
    "        error('set_point_light with cast_shadow failed')\n"
    "    end\n"
    "    local _, _, _, _, _, cast2 = engine.get_point_light(g_lit)\n"
    "    if cast2 ~= true then error('point cast_shadow did not read back') end\n"
    "    if not engine.add_spot_light(g_lit, 1.0, 0.5, 0.25, 0.0, -1.0, 0.0,\n"
    "                                 3.0, 9.0, 0.25, 0.5, true) then\n"
    "        error('add_spot_light failed')\n"
    "    end\n"
    "    local sr, sg, sb, dx, dy, dz, si, srad, inner, outer, scast =\n"
    ...
    "       or outer ~= 0.5 or scast ~= true then\n"
...
constexpr const char *kModule =
    ...
    "    end\n"
    "    -- The queued add is visible to the getter and setter in the same\n"
    "    -- callback (#574): read-through, like every other component.\n"
    "    local r, g, b, intensity, radius = engine.get_point_light(self)\n"
    "    if r == nil or radius ~= 11.0 or intensity ~= 4.0 then\n"
    "        error('get_point_light did not see the queued add')\n"
    "    end\n"
    "    if not engine.set_point_light(self, r, g, b, 5.0, 12.0) then\n"
    "        error('set_point_light did not see the queued add')\n"
    "    end\n"
    "    local _, _, _, intensity2 = engine.get_point_light(self)\n"
    "    if intensity2 ~= 5.0 then\n"
    "        error('get_point_light did not see the queued set')\n"
    "    end\n"
    "    -- Back to the values the flush assertions below expect.\n"
    "    if not engine.set_point_light(self, r, g, b, 4.0, 11.0) then\n"
    "        error('second set_point_light failed')\n"
    "    end\n"
    "    if not engine.remove_spot_light(self) then\n"
    "        error('queued remove_spot_light failed')\n"
    "    end\n"
    "    if engine.get_spot_light(self) ~= nil then\n"
    "        error('get_spot_light must read the queued removal')\n"
    "    end\n"
    "    bp_completed = true\n"
    "end\n"
    "return M\n";
