...
vec3 preetham_sky(vec3 direction, vec3 sunDirection, float turbidity) {
    ...
    float zenithX =
        ...
        (-0.02902 * thetaS3 + 0.06377 * thetaS2 - 0.03202 * thetaS +
         0.00394) *
            T +
...
void main() {
    vec3 direction = normalize(v_dir);
    vec3 skyColor =
        preetham_sky(direction, u_sunDirection.xyz, u_turbidity.x);
