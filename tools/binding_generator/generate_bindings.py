...
# Registers one generated binding, refusing a name a hand-written wrapper
# already registered: two owners for one Lua name means one silently
# replaces the other, and fixes to the shadowed one never take effect.
# That is a programmer error, so it aborts in every build and no test that
# starts scripting can pass with a collision in place.
SET_GENERATED = """void set_generated(lua_State* L, const char* name,
                   lua_CFunction fn) noexcept {
  lua_getfield(L, -1, name);
  const bool unclaimed = lua_isnil(L, -1);
  lua_pop(L, 1);
  if (!unclaimed) {
    std::fprintf(stderr,
                 "engine.%s is registered by hand and generated from "
                 "LUA_BIND; give it one owner\\n",
                 name);
  }
  ENGINE_ASSERT(unclaimed);
...
def generate_registration(bindings):
    ...
    for b in bindings:
        lua_name = b['lua_name']
        lines.append(f'  set_generated(L, "{lua_name}", &lua_generated_{lua_name});')
    lines.append('}')
    return '\n'.join(lines)
...
def generate_output(bindings, input_headers):
    ...
    parts.append('')
    parts.append('#include "engine/core/assertion.h"')
    parts.append('')
